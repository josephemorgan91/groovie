...
tmdb = {
	...
	getMovieDataByKeyword : function(keyword) {
		return new Promise(function(resolve, reject) {
			let url = ''.concat(tmdb.baseURL, 'search/movie?api_key=', API_KEY, '&query=', keyword);
			fetch(url)
				.then((result) => {
					return result.json();
				})
					...
					if (data.results) {
						...
						for (let i = 0; i < data.results.length; ++i) {
							...
							if (data.results[i].poster_path === null) {
								tmdb.movieData[i].posterUrl = tmdb.defaultMoviePosterUrl;
							}
